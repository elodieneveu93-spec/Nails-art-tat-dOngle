...
export default function Home() {
  ...
  function exportPDF() {
    ...
    doc.text("Brief – Kit Press-On personnalisé", margin, y); y += 22;
    ...
    doc.setFont("helvetica", "bold"); doc.text("Tailles (gauche → droite)", margin, y); y += 16;
    doc.setFont("helvetica", "normal");
    ["Pouce","Index","Majeur","Annulaire","Auriculaire"].forEach((f,i)=>{
      doc.text(`${f}: G ${sizesLeft[i] || "—"} | D ${sizesRight[i] || "—"}`, margin, y); y += 14;
    });

    if (notes) {
      y += 10; doc.setFont("helvetica", "bold"); doc.text("Notes", margin, y); y += 14;
      doc.setFont("helvetica", "normal");
      const notesSplit = doc.splitTextToSize(notes, 515);
      doc.text(notesSplit, margin, y); y += notesSplit.length * 14 + 6;
    }

    if (images.length) {
      y += 8; doc.setFont("helvetica", "bold"); doc.text("Visuels (miniatures)", margin, y); y += 14;
  ...
  const sizeInput = (val, onChange) => (
    <input value={val} onChange={(e)=>onChange(e.target.value)} placeholder="0–10"
      style={{ width:70,border:"1px solid #e5e7eb",borderRadius:10,padding:6,fontSize:12 }}/>
  );

  return (
    <div style={{ maxWidth: 1100, margin: "24px auto", padding: 16, fontFamily: "sans-serif" }}>
      <h1 style={{ fontSize: 24, fontWeight: 700 }}>Crée ton kit <i>Press-On</i> personnalisé</h1>
      ...
      <div style={{ display: "grid", gap: 16, gridTemplateColumns: "1fr 1.2fr" }}>
        ...
        <div>
            ...
            <div style={{ display: "grid", gap: 8, gridTemplateColumns: "1fr 1fr" }}>
              {PRESETS.map((p) => (
                <button key={p.label} onClick={() => applyPreset(p)}
                  style={{ border: "1px solid #e5e7eb", borderRadius: 14, padding: 8, textAlign: "left", fontSize: 12 }}>
          ...
          <div style={{ marginTop: 10 }}>
            ...
            <input value={shape} onChange={(e)=>setShape(e.target.value)} placeholder="amande, coffin, ovale…"
          ...
          <div>
            ...
            <input value={length} onChange={(e)=>setLength(e.target.value)} placeholder="courte, moyenne, longue"
              style={{ width:"100%",border:"1px solid #e5e7eb",borderRadius:14,padding:8,marginTop:4 }}/>
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 700 }}>Couleurs</label>
            <input value={colors} onChange={(e)=>setColors(e.target.value)} placeholder="bleu pastel, nude rosé, noir mat…"
              style={{ width:"100%",border:"1px solid #e5e7eb",borderRadius:14,padding:8,marginTop:4 }}/>
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 700 }}>Motifs / déco</label>
            <input value={motifs} onChange={(e)=>setMotifs(e.target.value)} placeholder="fleurs fines, chrome, paillettes, french…"
              style={{ width:"100%",border:"1px solid #e5e7eb",borderRadius:14,padding:8,marginTop:4 }}/>
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 700 }}>Ambiance / style</label>
            <input value={vibe} onChange={(e)=>setVibe(e.target.value)} placeholder="doux et poétique, glam rock, minimal chic…"
              style={{ width:"100%",border:"1px solid #e5e7eb",borderRadius:14,padding:8,marginTop:4 }}/>
          </div>

          <button onClick={generate} disabled={loading}
            style={{ marginTop: 10, width: "100%", background: "#000", color: "#fff", border: "none", borderRadius: 14, padding: 10, fontWeight: 700 }}>
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <textarea readOnly value={prompt}
              style={{ width: "100%", height: 100, border: "1px solid #e5e7eb", borderRadius: 14, padding: 8, marginTop: 4, fontSize: 12 }}/>
          </div>
                ...
                <div style={{ border: "1px solid #e5e7eb", borderRadius: 14, padding: 8 }}>
                  ...
                  {["Pouce","Index","Majeur","Annulaire","Auriculaire"].map((f,i)=>(
                    <div key={f} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom: 6 }}>
                      ...
                      {sizeInput(sizesLeft[i], v => { const c=[...sizesLeft]; c[i]=v; setSizesLeft(c); })}
                    </div>
                  ))}
                </div>
                <div style={{ border: "1px solid #e5e7eb", borderRadius: 14, padding: 8 }}>
                  ...
                  {["Pouce","Index","Majeur","Annulaire","Auriculaire"].map((f,i)=>(
                    <div key={f} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom: 6 }}>
                      <span style={{ fontSize: 12, width: 80 }}>{f}</span>
                      {sizeInput(sizesRight[i], v => { const c=[...sizesRight]; c[i]=v; setSizesRight(c); })}
                    </div>
                  ))}
                </div>
              </div>
